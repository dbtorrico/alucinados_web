...
class login extends index {
    ...
    private function validarUsuario($login, $password) {
        ...
        if ($bd->getQuantidadeRegistros() == 1) {
            ...
            echo"<script>
                    alert('Atenção, este usuário esta replicado no Banco de Dados.')
                </script>";
    ...
    private function executarLogon($login_email, $password) {
        if ($this->validarUsuario($login_email, $password)) {
            $bd = new BD();
            //nome, pessoa_id e email (login) do usuario
            $sql = "SELECT pessoa.id_pessoa, pessoa.nome, pessoa.email FROM pessoa WHERE pessoa.email = '$login_email'";
            $bd->executarSQL($sql);
            $row = $bd->getObjeto();
            if ($bd->getQuantidadeRegistros() > 0) {
                ...
                if (in_array($login_email, array('admin')))
                    $_SESSION['role'][] = 'administrador';
                echo 'administrador!';

                //se nao tem papel entao retorna false
                if (empty($_SESSION['role'])) {
                    $_SESSION['erro'] = "O usuário $login_email não tem permissão dentro deste sistema.";
                    return false;
                }
                ...
                setcookie('usuario', $login_email, time() + 3600 * 24 * 30); //cria cookie para salvar o login do usuario para a proxima vez (na mesma maquina) nao ser necessario digitar o login
                //permite logon
                return true;
            } else {
                $_SESSION['erro'] = "O usuário $login_email está inativo ou não existe seu cadastro como pessoa.";
                return false;
            }
        } else {
