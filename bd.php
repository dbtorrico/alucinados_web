 ...
 *
 * @author NTI/UNIFAL-MG (Gustavo Afonso) <gustavo.afonso@unifal-mg.edu.br>
 */
class BD
{
     ...
     */
     public function __construct($BD='unifalatual', $HOST='200.131.224.14', $USUARIO='postgres',$PWD='postgres', $SGBD='postgres', $PORTA='15432'){
        $this->BD = $BD;
        $this->HOST = $HOST;
    ...
    {
        if($this->SGBD == 'mysql') return @mysql_fetch_object($this->resultado);
        if($this->SGBD == 'postgres') return @pg_fetch_object($this->resultado);
    }
    ...
    {
        if($this->SGBD == 'mysql') return @mysql_fetch_row($this->resultado);
        if($this->SGBD == 'postgres') return @pg_fetch_row($this->resultado);
    }
