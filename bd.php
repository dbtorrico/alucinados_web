...
class BD {
    ...
    public function bdInserir() {
        $sql = "INSERT INTO patrimonio.termoresponsabilidade(bemmovel_id, servidor_id, unidade_id, data, identificacaodocumento, status)
        VALUES('$this->bemmovel_id', '$this->servidor_id', '$this->unidade_id', '$this->data', '$this->identificacaodocumento', '$this->status') RETURNING id";
        $this->bd->executarSQL($this->bd->substituiVazioPorNull($sql));
        $obj = $this->bd->getObjeto();
        if (!empty($obj->id)) {
            $this->bd->log('inserir termo de responsabilidade', $sql, $obj->id); //gera log
            $this->id = $obj->id; //atribuindo o id inserido ao objeto da classe TermoResponsabilidade
            //atualizando o status do termo anterior(es) para INATIVO
            $sql = "UPDATE patrimonio.termoresponsabilidade SET 
                        status = 'TRANSFERIDO'
                     WHERE id <> $this->id AND bemmovel_id = '$this->bemmovel_id' AND status = 'ATIVO'";
